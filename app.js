// ==== Minimal full implementation ====
const COLORS=[{name:'橙',value:'#FF7A00'},{name:'黑',value:'#111111'},{name:'奶黄',value:'#FFE9A8'},{name:'粉',value:'#FF7FB0'},{name:'香槟米',value:'#F7E7CE'},{name:'苔绿',value:'#597D4A'},{name:'酒红',value:'#7F1D2D'},{name:'天蓝',value:'#86C5FF'}];
const STYLES=[{id:'moon-rabbit',name:'Moon Rabbit'},{id:'clean-fit',name:'Clean Fit'},{id:'twill',name:'Twill'},{id:'racing',name:'Racing'},{id:'sports',name:'Sports'},{id:'mountain',name:'Mountain'},{id:'ballet',name:'Ballet Girl'},{id:'y2k',name:'Y2K'}];
...
const bind=()=>{
  const colorDot=$('colorDot'); const colorSwatches=$('colorSwatches'); const styleGallery=$('styleGallery');
  const segBtns=[...document.querySelectorAll('.seg-btn')];
  const tabs=[...document.querySelectorAll('.tab')];
  const tIn=$('customText'),tSize=$('textSize'),tW=$('textWeight'),loop=$('loopText');
  const bty=$('beautyToggle'),ar=$('arToggle');
  const help=$('help'); $('helpBtn').onclick=()=>help.classList.remove('hidden'); $('closeHelp').onclick=()=>help.classList.add('hidden');
  const setColor=c=>{S.bandColor=c;colorDot.style.background=c}; setColor(S.bandColor);
  COLORS.forEach(c=>{const b=document.createElement('button'); b.title=c.name;b.style.background=c.value;b.onclick=()=>setColor(c.value);colorSwatches.appendChild(b)});
  STYLES.forEach(s=>{const b=document.createElement('button'); b.innerHTML=`<span>${s.name}</span>`; b.onclick=()=>S.styleId=s.id; styleGallery.appendChild(b)});
  ...
  bty.onclick=()=>{S.beautyOn=!S.beautyOn;bty.classList.toggle('on',S.beautyOn)}; ar.onclick=()=>{S.bubblesOn=!S.bubblesOn;ar.classList.toggle('on',S.bubblesOn)};
};
async function startCam(){ if(V.srcObject) V.srcObject.getTracks().forEach(t=>t.stop());
  try{V.srcObject=await navigator.mediaDevices.getUserMedia({video:{facingMode:S.facingMode,width:{ideal:1280},height:{ideal:720}},audio:false});}
  catch(_){V.srcObject=await navigator.mediaDevices.getUserMedia({video:true,audio:false});}
  await V.play(); resize();}
...
async function tickFM(){ if(!V.videoWidth){requestAnimationFrame(tickFM);return;} await FM.send({image:V}); requestAnimationFrame(tickFM);}
...
function drawBand(rect){const {x,y,w,h,angle}=rect;CTX.save();CTX.translate(x,y);CTX.rotate(angle);
  ...
  CTX.fillStyle='#fff'; CTX.strokeStyle='rgba(0,0,0,.25)'; CTX.lineWidth=4; CTX.font=`bold ${S.textSize}px Inter, system-ui, -apple-system`; CTX.textAlign='center';
...
function drawB(){for(let i=bubbles.length-1;i>=0;i--){const b=bubbles[i]; b.x+=b.vx;b.y+=b.vy;b.a-=.012;b.r*=1.003; if(b.a<=0){bubbles.splice(i,1);continue;}
  ...
  if(logo.complete){const s=b.r*1.4; CTX.save(); CTX.translate(b.x,b.y); CTX.rotate(b.rot); CTX.drawImage(logo,-s/2,-s/2,s,s); CTX.restore();} CTX.restore(); }}
...
share.onclick=async()=>{try{const res=await fetch(img.src); const blob=await res.blob(); const file=new File([blob],'headband.png',{type:'image/png'}); if(navigator.canShare&&navigator.canShare({files:[file]})) await navigator.share({files:[file],title:'Headband'}); else alert('当前浏览器不支持原生分享，可直接下载图片。')}catch(e){alert('分享失败，请先下载图片。')}};

// boot
(async()=>{bind(); await startCam(); const bOn=document.getElementById('beautyToggle'); V.style.filter=S.beautyOn?'brightness(1.06) contrast(1.04) saturate(1.05)':'none'; bOn.onclick=()=>{S.beautyOn=!S.beautyOn; bOn.classList.toggle('on',S.beautyOn); V.style.filter=S.beautyOn?'brightness(1.06) contrast(1.04) saturate(1.05)':'none'}; await tickFM(); render();})();
